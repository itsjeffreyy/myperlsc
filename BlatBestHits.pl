...
while(<IN>) {
    ...
    my ($m,$mm,$qgb,$tgb,$qs,$qsl,$qas,$qae,$ts)=@a[0,1,5,7,9,10,11,12,13];

    my @qar=();
    my %ara=();

    # keep only the alignments whose total number of mis-match and gap bases is smaller than the number of matches
    # and the ratio of mismatches to matches is smaller than a cutoff
#    if($qs eq "contig00024" && $m<400) {
#	next;
#    }
#    if($qs eq "contig00001" && $m<1000) {
#	next;
#    }
    if(($mm+$qgb+$tgb)<=$m && ($mm/$m)<=$mmrmax) {
	$qas++;
	push(@qar,"$qas~$qae");
	push(@{$ara{"$qas~$qae"}},$_);
	push(@{$tsa{$ts}},$_);
	...
	if($a[9] eq $qs) {
	    ($m,$mm,$qgb,$tgb,$qs,$qsl,$qas,$qae,$ts)=@a[0,1,5,7,9,10,11,12,13];
...
sub BestAlignment {
    ...
    my @aln=sort{$am{$b}<=>$am{$a} || $ap{$b}<=>$ap{$a}}@_;
    return $aln[0];
}
